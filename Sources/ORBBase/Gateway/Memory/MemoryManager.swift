...
actor MemoryManager {
    ...
    init() {
        ...
        storageDir = appSupport.appendingPathComponent("ORBBase/memory", isDirectory: true)
...
struct MemoryKnowledge: Codable {
    ...
    static let `default` = MemoryKnowledge(
        ...
        projects: [
            MemoryProject(name: "ORB", summary: "Voice-first AI assistant iOS/macOS app. Main project — sacred, never break it.", status: "active", lastMentioned: Date()),
            MemoryProject(name: "ORB Base", summary: "macOS gateway server — routes LLM requests, manages API keys, runs Ollama", status: "active", lastMentioned: Date()),
