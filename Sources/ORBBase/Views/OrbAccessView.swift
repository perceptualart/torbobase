...
// The ORB replaces the dial — a living, breathing access control interface
// v2: Giant orb + rainbow slider + network health
import SwiftUI

/// The ORB access control — giant orb with rainbow slider beneath.
struct OrbAccessView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            // The ORB — massive, alive, the hero
            ZStack {
...
/// Live health status for all connected services.
/// Pings Ollama, Tailscale, cloud providers, memory system.
struct NetworkHealthView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                HealthSectionHeader(title: "NETWORK HEALTH")
                Spacer()
    ...
    private func runChecks() async {
        ...
        // 3. Embeddings model (nomic-embed-text)
        ...
        // 6-8. Cloud providers
        for provider in [("OpenAI", "openai"), ("Anthropic", "anthropic"), ("Google", "google")] {
    ...
    private func checkTailscale() async -> TailscaleResult {
        ...
        do {
            ...
            if process.terminationStatus == 0 {
                ...
                statusProc.executableURL = URL(fileURLWithPath: "/usr/local/bin/tailscale")
                ...
                do {
                    ...
                    if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
                        ...
                        if backendState == "Running" {
                            ...
                            return TailscaleResult(status: .ok, latency: selfIP.isEmpty ? "" : String(selfIP.prefix(15)), detail: "Tailscale connected")
                        } else {
                            return TailscaleResult(status: .warning, latency: "", detail: "State: \(backendState)")
                        }
                    }
                } catch {}
        ...
        return TailscaleResult(status: .error, latency: "", detail: "Tailscale not found")
    }
}
